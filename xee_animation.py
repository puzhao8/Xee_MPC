...
def group_ImgCol(imgcollection, group_level='monthly'):
    ...
    def mosaic_grouped_image(img):
        mosaiced = ee.ImageCollection.fromImages(img.get('to_mosaic')).mean() # mosaic or mean
        return ee.Image(mosaiced).updateMask(1).copyProperties(img,img.propertyNames()) #lost bounds maybe with mosaic operation
...
# https://ecmwf-projects.github.io/copernicus-training-cams/vis-animations.html
...
S5P_grouped = group_ImgCol(S5P_flt, 'monthly')
...
import xarray
ds = xarray.open_dataset(S5P_grouped, 
                         engine='ee', 
